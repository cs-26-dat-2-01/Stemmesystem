...
import { Poll, pollVisibility, ballotPrivacy } from "../WebLib.ts";
...
function CreatePollStep1 ({ onNext }: { onNext: (data: Poll) => void}) {
    ...
    function buildPollData(): Poll {
        return {
            ...
            startsAt: startNow ? new Date().toISOString() : `${startDate}T${startsAt}`,
...
function CreatePollStep2({ onNext, voters, setVoters }: {
    ...
    return (
        <div className="create-poll-content">
            ...
            <p className="field-hint">Tilføj de personer som må stemme i afstemningen.</p>
                ...
                {voters.length === 0
                    ...
                    : voters.map((voter, i) => (
                        <div key={i} className="voter-row">
                            <span>{voter}</span>
                            <button type="button" onClick={() => handleRemove(i)}>Fjern</button>
                        </div>
                    ))
                }
            </div>
...
function CreatePollStep3({ onNext, choices, setChoices }: {
    ...
    return (
        <div className="create-poll-content">
            ...
            <p className="field-hint">Tilføj de valgmuligheder som stemmeberettigede kan stemme på.</p>
            ...
            <br />
            <button type="button" className="button-secondary" onClick={handleAddChoice}>
                + Tilføj valgmulighed
            </button>
            ...
            <div className="create-poll-grid-2">
                ...
                <div>
                    ...
                    <div className="overview-field">
                        ...
                        <span>{pollData.pollVisibility === "public" ? "Offentlig" : "Privat"}</span>
                    </div>
                    <div className="overview-field">
...
function CreatePollPage({ onExit }: { onExit:() => void }) {
    ...
    return (
        <div className="create-poll-page">
            ...
            {step === 1 && <CreatePollStep2 onNext={() => setStep(2)} voters={voters} setVoters={setVoters} />}
            {step === 2 && <CreatePollStep3 onNext={() => setStep(3)} choices={choices} setChoices={setChoices} />}
            {step === 3 && <CreatePollStep4 onNext={onExit} pollData={pollData} voters={voters} choices={choices} />}
            ...
            <div className="create-poll-nav">
                ...
                <div className="create-poll-nav-steps">
                    <button type="button" className="button-secondary" onClick={() => setStep(s => Math.max(0, s - 1))}>«</button>
                    {steps.map((s, i) => (
                        <button
                            key={i}
                            type="button"
                    ...
                    <button type="button" className="button-secondary" onClick={() => setStep(s => Math.min(3, s + 1))}>»</button>
                </div>
