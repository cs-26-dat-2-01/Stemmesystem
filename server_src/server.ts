 ...
 */
  ...
  */
  ...
  // ─── Admin routes ──────────────────────────────────────────────────────────
  // All /admin/* routes require a valid JWT and username must be "admin".

  // GET /admin/users — returns list of all users (without passwordHash)
  router.get("/admin/users", async (c) => {
    return await hasValidJWT(c, async (payload) => {
      if (payload.username !== "admin") return c.body("401 Unauthorized", 401);
  ...
  router.post("/admin/users", async (c) => {
    return await hasValidJWT(c, async (payload) => {
      if (payload.username !== "admin") return c.body("401 Unauthorized", 401);
      let body;
      try { body = await c.req.json(); } catch { return c.body("Ugyldigt JSON", 400); }
      const { username, password } = body;
      if (!username || !password) return c.body("Brugernavn og adgangskode er påkrævet", 400);
      const status = await DB.addUserToDB(username, password);
      return c.body("", status);
    });
  });

  // DELETE /admin/users/:username — deletes a user
  router.delete("/admin/users/:username", async (c) => {
    return await hasValidJWT(c, async (payload) => {
      if (payload.username !== "admin") return c.body("401 Unauthorized", 401);
      const username = c.req.param("username");
      // Protect the admin account from being deleted via the dashboard
      if (username === "admin") return c.body("Admin-kontoen kan ikke slettes", 403);
      const user = await DB.getUserFromDB(username);
      if (user.httpStatusCode !== 200) return c.body("Bruger ikke fundet", 404);
      await DB.deleteUserFromDB(username);
