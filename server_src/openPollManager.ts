...
const closeCommitment = buildCloseCommitment(pollResult.poll, votes.votes, closedAt); 
 let closeTimestampQuery: Uint8Array;
    ...
    const finalizeResult = await this.DB.setCloseArtifacts(pollId, closeCommitment, closeTimestampQuery, closeTimestampToken, closedAt); 
 ...
 this.DB.insertAuditLog(
	...
	public async castVoteOpen(
		...
		if (votes.length <= 0){
			return {success: false, errorMsg: "Invalid no votes", httpStatusCode: 400};
		}
...
if (poll.httpStatusCode !== 200 || !poll.poll) {
	...
	pos += 1; 
	const currentHash = createVoteHash(prev, uuid, optionId, userId, pollId, pollData.ballotPrivacy, pollData.showTopN); 
	built.push({optionId, uuid, userId, signature: null, chainPosition: pos, previousHash: prev, currentHash}); 
	prev = currentHash; 
	};
