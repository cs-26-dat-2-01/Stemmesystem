...
export class WebappDatabase {
  ...
  /**
   * Returnerer alle brugere i databasen uden passwordHash.
   * Bruges af admin-dashboardet til at vise brugerlisten.
   */
  public getAllUsers(): { id: number; username: string }[] {
    ...
    return rows
      .filter(
        (row) => typeof row.id === "number" && typeof row.username === "string",
      )
      .map((row) => ({ id: row.id as number, username: row.username as string }));
  }
